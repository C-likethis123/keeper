...
function createGetColorForClass(theme: Omit<SyntaxTheme, 'getColorForClass'>): SyntaxTheme['getColorForClass'] {
  return (className: string | null) => {
    ...
    if (className.includes('type') || className.includes('class')) return theme.typeOfVariable;

	...
	return {
		promises: {
			...
			async mkdir(
				...
				if (dir.exists)
					throw errWithCode(`Directory exists: ${dirPath}`, "EEXIST");
			...
			async rmdir(dirPath: string): Promise<void> {
				...
				const dir = new Directory(path);
				if (!dir.exists)
					throw errWithCode(`Directory not found: ${dirPath}`, "ENOENT");
			...
			async readdir(dirPath: string): Promise<string[]> {
				...
				const file = new File(path);
				if (file.exists)
					throw errWithCode(`Not a directory: ${dirPath}`, "ENOTDIR");
				if (!dir.exists)
					throw errWithCode(`Directory not found: ${dirPath}`, "ENOENT");
			...
			async stat(filePath: string) {
				...
				const mtimeMs =
					isFile && file.modificationTime != null ? file.modificationTime : 0;
			...
			async rename(oldPath: string, newPath: string): Promise<void> {
				...
				if (!source.exists)
					throw errWithCode(`File not found: ${oldPath}`, "ENOENT");
			...
			async rm(
				...
				if (dir.exists) {
					if (options?.recursive) {
						...
						throw errWithCode(
							`Cannot remove directory without recursive option: ${filePath}`,
							"EISDIR",
						);
					}
				} else if (file.exists) {

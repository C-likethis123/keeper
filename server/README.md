...
```bash
KEEPER_CORS_ALLOWED_ORIGINS=https://keeper.pages.dev
KEEPER_SYNC_BODY_LIMIT_BYTES=16777216
KEEPER_RATE_LIMIT_MAX=120
KEEPER_RATE_LIMIT_WINDOW_MS=60000
```

Origins must be exact origins without paths. Add local browser origins explicitly
for development. Requests without an `Origin` header, including native app

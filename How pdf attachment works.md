Attaching starts from the toolbar paperclip button. In browser editor, that button opens
an `<input type="file">`, accepts only PDF or EPUB, stores bytes in IndexedDB, then saves
returned `_attachments/...` path on note. UI opens split view immediately. Native path
continues to use `expo-document-picker`. Frontmatter also supports attachment path.
  ...
  non-null, it renders a two-pane layout with the document panel on the left for wide
  browser layouts and on top for mobile, with a draggable divider clamped to 25% to 75% in src/
  ...
  path into a Blob object URL first. On web, PDFs are shown by dropping that URL
  into an <iframe> in src/components/editor/document/DocumentPanel.web.tsx,
  so browser does rendering natively. On native, PDFs go through a

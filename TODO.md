...
- **Next keyboard work**: soft line breaks, better code-block vertical navigation, formatting shortcuts, app-level shortcuts
...
- **Testing TODO**: Add missing Jest coverage for `HybridEditor`, `EditorToolbar`, `useAutoSave`, `useLoadNote`, startup UI/runtime flows, Wikilink interactions, desktop scroll/page coordination, `src/app/index.tsx`/`NoteGrid`, and additional `editorStore` flows; migrate remaining `vitest` suites to `jest` over time for consistency
- **Known regression to investigate**: desktop scroll-to-end does not advance to the next page, even though mobile still does

...
- **Phase 6**: In progress; `jest-expo` + React Native Testing Library are in place, with coverage for `src/app/editor.tsx`, `NoteEditorView`, and focused `NoteGrid` load-more behavior
- **Next keyboard work**: soft line breaks, better code-block vertical navigation, cursor selection fixes, code-block brace auto-completion fixes, formatting shortcuts, app-level shortcuts
...
- **Backlog**: add note templates so new notes can start from a template and users can create reusable templates

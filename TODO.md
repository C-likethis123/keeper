...
- **Phase 6**: In progress; `jest-expo` + React Native Testing Library are in place, with coverage for `src/app/editor.tsx`, `src/app/index.tsx`, `NoteEditorView`, `NoteFiltersDropdown`, focused `NoteGrid` load-more behavior, and wikilink modal/overlay flows
...
- **Testing status**: `npm test` covers the immutable editor core plus `EditorState`, selected `editorStore` flows, `frontmatter`, `repoBootstrapper`, `startupSteps`, `src/app/editor.tsx`, `src/app/index.tsx`, `NoteEditorView`, `NoteFiltersDropdown`, focused `NoteGrid` pagination behavior, and wikilink modal/overlay interactions
- **Testing TODO**: Add missing Jest coverage for `HybridEditor`, `EditorToolbar`, `useAutoSave`, `useLoadNote`, startup UI/runtime flows, broader `NoteGrid` and note-list states, full editor-level wikilink activation/navigation, and additional `editorStore` flows

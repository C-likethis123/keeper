...
export class SmartEditingHandler {
	...
	private get bracePairs(): Record<string, string> {
		return (
			this.languageConfig?.bracePairs || SmartEditingHandler.defaultBracePairs
		);
	}
	...
	handleBackspace(text: string, cursorOffset: number): SmartEditResult {
		...
		if (this.bracePairs[charBefore]) {
			...
			if (cursorOffset < text.length && text[cursorOffset] === closingChar) {
				const newText =
					text.substring(0, cursorOffset - 1) +
	...
	handleEnter(text: string, cursorOffset: number): SmartEditResult {
		...
		if (
			...
			SmartEditingHandler.indentTriggers.has(
		...
		if (
			...
			SmartEditingHandler.outdentTriggers.has(afterCursor[0])
		) {
	...
	private handleOutdent(text: string, cursorOffset: number): SmartEditResult {
		...
		if (currentLine.startsWith(this.indentString)) {
			...
			return {
				newText,
				newCursorOffset: Math.max(
					lineStart,
					Math.min(text.length, cursorOffset - this.indentString.length),
				),
				handled: true,
			};
			...
			return {
				...
				newCursorOffset: Math.max(
					lineStart,
					Math.min(text.length, cursorOffset - 1),
				),
				handled: true,
			};
		}
	...
	private shouldAutoCompleteQuote(
		text: string,
		cursorOffset: number,
		quote: string,
	): boolean {
